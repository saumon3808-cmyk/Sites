...
async function parseClanData(
  url: string,
): Promise<Partial<ClanData>> {
  try {
  ...
  try {
    ...
    for (const clan of clanLinks) {
      try {
        ...
        console.error(
          `Ошибка обновления данных для клана ${clan.tag}:`,
          error,
        );
    ...
    console.error(
      "Ошибка при обновлении данных кланов:",
      error,
    );
    return c.json(
      {
...
app.post(
  "/make-server-f9a5a1a1/update-clan/:tag",
  async (c) => {
    try {
